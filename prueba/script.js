...
const stationsData = [
    ...
    // --- MODIFICACIÓN: AÑADIDO isCasterFm y publicToken, SE REMOVIÓ LA URL ESTÁTICA ---
    { id: 55, name: "Radio Pizza Acer Aspire One", isCasterFm: true, publicToken: "9bb3e53c-efc7-47a1-bccc-089c202abdfb" }
];
...
// --- REPRODUCTOR ---
// --- MODIFICACIÓN: LÓGICA AGREGADA PARA RESOLVER TOKEN DE CASTER.FM ---
function selectStation(station) {
    ...
    } else if (station.isCasterFm) {
        if(ytReady) ytPlayer.stopVideo(); // Detiene YT si pasas a FM
        
        showToast("Conectando con Radio Pizza...");
        
        // Hacemos la consulta a la API usando tu token público para obtener los datos frescos
        fetch(`https://hub.cloud.caster.fm/public/accountInfo?token=${station.publicToken}`)
            .then(res => {
                if(!res.ok) throw new Error("Error conectando con la API de Caster.fm");
                return res.json();
            })
            .then(data => {
                // Verificamos que el usuario no haya cambiado de emisora rápido antes de que cargara la petición
                if (currentStation.id !== station.id) return;
                
                // Extraemos la IP, Puerto y Mount del JSON que devuelve tu token público.
                // Usamos los datos originales de tu URL como respaldo seguro.
                const ip = data.server_ip || data.server || "sapircast.caster.fm";
                const port = data.port || data.server_port || "11841";
                const mount = data.mount || data.mount_point || "/n7v8d";
                
                // Caster a veces entrega la URL directa en el JSON (ej. stream_url). 
                // Si no, la construimos dinámicamente con los parámetros frescos.
                const streamUrl = data.stream_url || data.url || `https://${ip}:${port}${mount}`;
                
                audio.src = streamUrl;
                playAudio();
            })
            .catch(err => {
                console.error("Error al obtener stream de Caster.fm:", err);
                showToast("Error: No se pudo conectar con Caster.fm");
                stopAudio();
            });
    } else {
        if(ytReady) ytPlayer.stopVideo(); // Detiene YT si pasas a FM
