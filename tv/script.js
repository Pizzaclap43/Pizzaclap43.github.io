...
const channels = [
    ...
    // Agregamos la propiedad audioUrl al objeto de Canal I
...
let hlsInstance;
let hlsAudioInstance; // Nueva variable para controlar el stream de audio
const tvAudio = new Audio(); // Creamos un elemento de audio invisible en memoria
...
// --- EVENTOS DE SINCRONIZACIÓN DE AUDIO Y VIDEO ---
// Esto hace que el audio invisible responda a los controles del video
tvPlayer.addEventListener('play', () => tvAudio.play());
tvPlayer.addEventListener('pause', () => tvAudio.pause());
tvPlayer.addEventListener('waiting', () => tvAudio.pause()); // Pausa el audio si el video se queda cargando buffer
tvPlayer.addEventListener('playing', () => tvAudio.play());
...
window.playChannel = function(id) {
    ...
    // Limpiar reproducciones anteriores (Video y Audio)
    tvAudio.pause();
    tvAudio.src = '';
    ...
    if (Hls.isSupported()) {
        ...
        // 2. Cargar el Audio (solo si el canal tiene la propiedad audioUrl)
        if (channel.audioUrl) {
            hlsAudioInstance = new Hls();
            hlsAudioInstance.loadSource(channel.audioUrl);
            hlsAudioInstance.attachMedia(tvAudio);
            hlsAudioInstance.on(Hls.Events.MANIFEST_PARSED, () => tvAudio.play());
        }
        ...
        if (channel.audioUrl) {
            ...
            tvAudio.addEventListener('loadedmetadata', () => tvAudio.play());
        }
    }
};
